...
# Test Delay
testPing(){
echo "\n"${skyBlue} Checking $5 ${none} ${red} Please wait ${none}"\n"
# 计算ip段
...
echo "${skyBlue}\n${indexLength} IPs Detected in total , Optimal IP in range of $5 is: ${none}"${green}$min${none}
}
manageFun(){
    case $1 in
        1)
            echo ${skyBlue}"Detected IP Segment: 104.23.240.0-104.23.243.254"${none}
            ipList=()
        ...
        ;;
        2)
            echo ${skyBlue}"Detected IP Segment: 1.0.0.0-1.0.0.254，1.1.1.0-1.1.1.254，104.16.80.0-104.16.95.255，104.16.175.255-104.16.191.255"${none}
        ...
        ;;
        3)
            echo ${skyBlue}"Detected IP Segment: 104.16.0.0-104.16.79.255，104.16.96.0-104.16.175.254，104.16.192.0-104.16.207.255"${none}
...
start(){
    echo "${purple}IP Segment, Cloudflare Delay${none}"
    echo "${purple}===============================${none}"
    echo "${green}  1.Unicom ${none}"
    echo "${green}  2.Mobile ${none}"
    echo "${green}  3.HK Direct Connection【Suitable for mobile&server】${none}"
    echo "${purple}===============================${none}"
    echo "${skyBlue}Please Select (1-3):${none} "
    read -e funType
    manageFun $funType
}
start
